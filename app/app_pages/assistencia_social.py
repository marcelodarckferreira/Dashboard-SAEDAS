...
from app.utils.schemas import SCHEMA_ASSISTENCIA_SOCIAL, SCHEMA_ASSISTENCIA_SOCIAL_ALUNO, SCHEMA_ASSISTENCIA_SOCIAL_ANO
...
def carregar_dados_assistencia_social():
    csv_file = "data/DashboardAssistenciaSocial.csv"
    df, info = load_csv(csv_file, expected_cols=SCHEMA_ASSISTENCIA_SOCIAL)

    csv_file_aluno = "data/DashboardAssistenciaSocialAluno.csv"
    df_aluno_raw, info_aluno = load_csv(
        csv_file_aluno, expected_cols=SCHEMA_ASSISTENCIA_SOCIAL_ALUNO
    )

    csv_file_ano = "data/DashboardAssistenciaSocialAno.csv"
    df_ano, info_ano = load_csv(csv_file_ano, expected_cols=SCHEMA_ASSISTENCIA_SOCIAL_ANO)
...
def page_assistencia_social():
    init_global_state()
    apply_global_css()

    st.title("Visão Geral dos Atendimentos de Assistente Social")
    st.markdown(
        "Resumo consolidado dos atendimentos de assistencia_social realizados por ano, URG e escola."
    )
    st.markdown(
        """
        <style>
.home-metric-label {
            ...
            .st-key-assistencia_social_urg_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-escola_table_selection_assistencia_social_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-assistencia_social_cobertura_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-assistencia_social_aluno_actions_toolbar div[data-testid="stHorizontalBlock"] {
                gap: 0 !important;
            ...
            .st-key-assistencia_social_urg_actions_toolbar div[data-testid="stColumn"],
            .st-key-escola_table_selection_assistencia_social_actions_toolbar div[data-testid="stColumn"],
            .st-key-assistencia_social_cobertura_actions_toolbar div[data-testid="stColumn"],
            .st-key-assistencia_social_aluno_actions_toolbar div[data-testid="stColumn"] {
                padding: 0 !important;
                margin: 0 !important;
            ...
            .st-key-assistencia_social_urg_actions_toolbar button,
            .st-key-escola_table_selection_assistencia_social_actions_toolbar button,
            .st-key-assistencia_social_cobertura_actions_toolbar button,
            .st-key-assistencia_social_aluno_actions_toolbar button {
            ...
            .st-key-assistencia_social_urg_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-escola_table_selection_assistencia_social_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-assistencia_social_cobertura_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-assistencia_social_aluno_actions_toolbar div[class*="st-key-copy_"] button {
                border-radius: 6px 0 0 6px !important;
            }
            .st-key-assistencia_social_urg_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-escola_table_selection_assistencia_social_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-assistencia_social_cobertura_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-assistencia_social_aluno_actions_toolbar div[class*="st-key-download_"] button {
    ...
    datasets = carregar_dados_assistencia_social()
    ...
    st.sidebar.title("Filtros - Assistente Social")
    ...
    sync_sidebar_escola_selection("escola_table_selection_assistencia_social")
    ...
    def toggle_atendimento(label: str) -> None:
        ...
        current = list(st.session_state.get("assistencia_social_atendimento_multiselect", []))
        if label in current:
            current.remove(label)
        else:
            current.append(label)
        st.session_state["assistencia_social_atendimento_multiselect"] = current
    ...
    if not df_home.empty:
        ...
        if "QtdAluno" in df_home.columns:
            df_home["QtdAluno"] = pd.to_numeric(df_home["QtdAluno"], errors="coerce").fillna(0)
            total_alunos_atendidos = int(df_home["QtdAluno"].sum())

    total_atend = df_filt["Quantidade"].sum() if not df_filt.empty else 0

    render_metric_cards([
        {"label": "TOTAL DE ALUNOS", "value": total_alunos},
        {"label": "ALUNOS ATENDIDOS", "value": total_alunos_atendidos},
        {"label": "ATEND. ASSIST. SOCIAL", "value": total_atend},
    ])
    ...
    if df_cmp_urg is not None:
        ...
        with st.container(key="assistencia_social_urg_actions_toolbar"):
            render_table_toolbar(df_cmp_urg_export, "performance_urg_assistencia_social.csv", "urg_table_assistencia_social")
    ...
    df_cmp_escola = render_top_por_urg(
        ...
        "Escola", 
        table_key="escola_table_selection_assistencia_social",
        active_row_value=st.session_state.get("sidebar_escola_filter", []),
        selection_mode="multiple"
    )

    escolas_tabela_atual = st.session_state.get("escola_table_selection_assistencia_social__selected_values", [])
    ...
    if df_aluno.empty:
        ...
        if aluno_col in df_aluno_filtrado.columns:
            ...
            alunos_selecionados = st.multiselect(
                ...
                placeholder="Todos",
                key="assistencia_social_aluno_multiselect",
            )
        ...
        if serie_col in df_aluno_filtrado.columns:
            ...
            series_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="assistencia_social_serie_multiselect",
            )
        ...
        if turma_col in df_aluno_filtrado.columns:
            ...
            turmas_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="assistencia_social_turma_multiselect",
            )
