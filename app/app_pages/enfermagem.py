...
.home-metric-label {
    ...
    sync_sidebar_escola_selection("escola_table_selection_enfermagem")
    ...
    render_metric_cards([
        ...
        {"label": "ATEND. ENFERMAGEM", "value": total_atend},
    ])
    ...
    if df_cmp_urg is not None:
        ...
        urg_table_key = f"urg_table_enfermagem_{_urg_key_sel}"
        _urg_key_changed = st.session_state.get("_is_page_first_run") or (st.session_state.get("_prev_urg_table_key_enfermagem") != urg_table_key)
        st.session_state["_prev_urg_table_key_enfermagem"] = urg_table_key
    ...
    if df_aluno.empty:
        ...
        if df_aluno_filtrado.empty:
            ...
            if not df_aluno_para_exibir.empty:
                ...
                render_aluno_detalhamento_aggrid(
                    df_aluno_head, 
                    key="aluno_table_enfermagem",
                    csv_name="detalhes_alunos_enfermagem.csv",
                    toolbar_key="enfermagem_aluno_actions_toolbar"
                )
                st.markdown("</div>", unsafe_allow_html=True)
            else:
