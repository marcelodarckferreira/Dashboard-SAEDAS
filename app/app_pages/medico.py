...
.home-metric-label {
    ...
    render_metric_cards([
        ...
        {"label": "ATEND. MÉDICO", "value": total_atend},
    ])
    ...
    if df_cmp_urg is not None:
        ...
        urg_table_key = f"urg_table_medico_{_urg_key_sel}"
        _urg_key_changed = st.session_state.get("_is_page_first_run") or (st.session_state.get("_prev_urg_table_key_medico") != urg_table_key)
        st.session_state["_prev_urg_table_key_medico"] = urg_table_key
    ...
    if df_aluno.empty:
        ...
        if df_aluno_filtrado.empty:
            ...
            if not df_aluno_para_exibir.empty:
                ...
                render_aluno_detalhamento_aggrid(
                    df_aluno_head, 
                    key="aluno_table_medico",
                    csv_name="detalhes_alunos_medico.csv",
                    toolbar_key="medico_aluno_actions_toolbar"
                )
                st.markdown("</div>", unsafe_allow_html=True)
            else:
