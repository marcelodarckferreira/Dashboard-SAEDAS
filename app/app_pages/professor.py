...
from app.utils.schemas import SCHEMA_PROFESSOR, SCHEMA_PROFESSOR_ALUNO, SCHEMA_PROFESSOR_ANO
...
def carregar_dados_professor():
    csv_file = "data/DashboardProfessor.csv"
    df, info = load_csv(csv_file, expected_cols=SCHEMA_PROFESSOR)

    csv_file_aluno = "data/DashboardProfessorAluno.csv"
    df_aluno_raw, info_aluno = load_csv(
        csv_file_aluno, expected_cols=SCHEMA_PROFESSOR_ALUNO
    )

    csv_file_ano = "data/DashboardProfessorAno.csv"
    df_ano, info_ano = load_csv(csv_file_ano, expected_cols=SCHEMA_PROFESSOR_ANO)
...
def page_professor():
    ...
    st.title("Visão Geral dos Atendimentos de Professor")
    st.markdown(
        "Resumo consolidado dos atendimentos de professor realizados por ano, URG e escola."
    )
    st.markdown(
        """
        <style>
.home-metric-label {
            ...
            .st-key-professor_urg_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-escola_table_selection_professor_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-professor_cobertura_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-professor_aluno_actions_toolbar div[data-testid="stHorizontalBlock"] {
                gap: 0 !important;
            ...
            .st-key-professor_urg_actions_toolbar div[data-testid="stColumn"],
            .st-key-escola_table_selection_professor_actions_toolbar div[data-testid="stColumn"],
            .st-key-professor_cobertura_actions_toolbar div[data-testid="stColumn"],
            .st-key-professor_aluno_actions_toolbar div[data-testid="stColumn"] {
                padding: 0 !important;
                margin: 0 !important;
            ...
            .st-key-professor_urg_actions_toolbar button,
            .st-key-escola_table_selection_professor_actions_toolbar button,
            .st-key-professor_cobertura_actions_toolbar button,
            .st-key-professor_aluno_actions_toolbar button {
            ...
            .st-key-professor_urg_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-escola_table_selection_professor_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-professor_cobertura_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-professor_aluno_actions_toolbar div[class*="st-key-copy_"] button {
                border-radius: 6px 0 0 6px !important;
            }
            .st-key-professor_urg_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-escola_table_selection_professor_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-professor_cobertura_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-professor_aluno_actions_toolbar div[class*="st-key-download_"] button {
    ...
    datasets = carregar_dados_professor()
    ...
    st.sidebar.title("Filtros - Professor")
    ...
    def toggle_atendimento(label: str) -> None:
        ...
        current = list(st.session_state.get("professor_atendimento_multiselect", []))
        if label in current:
            current.remove(label)
        else:
            current.append(label)
        st.session_state["professor_atendimento_multiselect"] = current
    ...
    render_metric_cards([
        ...
        {"label": "ATEND. PROFESSOR", "value": total_atend},
    ])
    ...
    if df_cmp_urg is not None:
        ...
        with st.container(key="professor_urg_actions_toolbar"):
            render_table_toolbar(df_cmp_urg_export, "performance_urg_professor.csv", "urg_table_professor")
        ...
        urg_table_key = f"urg_table_professor_{_urg_key_sel}"
        _urg_key_changed = st.session_state.get("_is_page_first_run") or (st.session_state.get("_prev_urg_table_key_professor") != urg_table_key)
        st.session_state["_prev_urg_table_key_professor"] = urg_table_key
    ...
    df_cmp_escola = render_top_por_urg(
        ...
        "Escola", 
        table_key="escola_table_selection_professor",
        active_row_value=st.session_state.get("sidebar_escola_filter", []),
        selection_mode="multiple"
    )

    escolas_tabela_atual = st.session_state.get("escola_table_selection_professor__selected_values", [])
    ...
    if df_aluno.empty:
        ...
        if aluno_col in df_aluno_filtrado.columns:
            ...
            alunos_selecionados = st.multiselect(
                ...
                placeholder="Todos",
                key="professor_aluno_multiselect",
            )
        ...
        if serie_col in df_aluno_filtrado.columns:
            ...
            series_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="professor_serie_multiselect",
            )
        ...
        if turma_col in df_aluno_filtrado.columns:
            ...
            turmas_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="professor_turma_multiselect",
            )
        ...
        if df_aluno_filtrado.empty:
            ...
            if not df_aluno_para_exibir.empty:
                ...
                render_aluno_detalhamento_aggrid(
                    df_aluno_head, 
                    key="aluno_table_professor",
                    csv_name="detalhes_alunos_professor.csv",
                    toolbar_key="professor_aluno_actions_toolbar"
                )
                st.markdown("</div>", unsafe_allow_html=True)
            else:
