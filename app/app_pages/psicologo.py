...
from app.utils.schemas import SCHEMA_PSICOLOGO, SCHEMA_PSICOLOGO_ALUNO, SCHEMA_PSICOLOGO_ANO
...
def carregar_dados_psicologo():
    csv_file = "data/DashboardPsicologo.csv"
    df, info = load_csv(csv_file, expected_cols=SCHEMA_PSICOLOGO)

    csv_file_aluno = "data/DashboardPsicologoAluno.csv"
    df_aluno_raw, info_aluno = load_csv(
        csv_file_aluno, expected_cols=SCHEMA_PSICOLOGO_ALUNO
    )

    csv_file_ano = "data/DashboardPsicologoAno.csv"
    df_ano, info_ano = load_csv(csv_file_ano, expected_cols=SCHEMA_PSICOLOGO_ANO)
...
def page_psicologo():
    ...
    st.title("Visão Geral dos Atendimentos de Psicólogo")
    st.markdown(
        "Resumo consolidado dos atendimentos de psicologo realizados por ano, URG e escola."
    )
    st.markdown(
        """
        <style>
.home-metric-label {
            ...
            .st-key-psicologo_urg_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-escola_table_selection_psicologo_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-psicologo_cobertura_actions_toolbar div[data-testid="stHorizontalBlock"],
            .st-key-psicologo_aluno_actions_toolbar div[data-testid="stHorizontalBlock"] {
                gap: 0 !important;
            ...
            .st-key-psicologo_urg_actions_toolbar div[data-testid="stColumn"],
            .st-key-escola_table_selection_psicologo_actions_toolbar div[data-testid="stColumn"],
            .st-key-psicologo_cobertura_actions_toolbar div[data-testid="stColumn"],
            .st-key-psicologo_aluno_actions_toolbar div[data-testid="stColumn"] {
                padding: 0 !important;
                margin: 0 !important;
            ...
            .st-key-psicologo_urg_actions_toolbar button,
            .st-key-escola_table_selection_psicologo_actions_toolbar button,
            .st-key-psicologo_cobertura_actions_toolbar button,
            .st-key-psicologo_aluno_actions_toolbar button {
            ...
            .st-key-psicologo_urg_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-escola_table_selection_psicologo_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-psicologo_cobertura_actions_toolbar div[class*="st-key-copy_"] button,
            .st-key-psicologo_aluno_actions_toolbar div[class*="st-key-copy_"] button {
                border-radius: 6px 0 0 6px !important;
            }
            .st-key-psicologo_urg_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-escola_table_selection_psicologo_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-psicologo_cobertura_actions_toolbar div[class*="st-key-download_"] button,
            .st-key-psicologo_aluno_actions_toolbar div[class*="st-key-download_"] button {
    ...
    datasets = carregar_dados_psicologo()
    ...
    st.sidebar.title("Filtros - Psicólogo")
    ...
    def toggle_atendimento(label: str) -> None:
        ...
        current = list(st.session_state.get("psicologo_atendimento_multiselect", []))
        if label in current:
            current.remove(label)
        else:
            current.append(label)
        st.session_state["psicologo_atendimento_multiselect"] = current
    ...
    render_metric_cards([
        ...
        {"label": "ATEND. PSICÓLOGO", "value": total_atend},
    ])
    ...
    if df_cmp_urg is not None:
        ...
        with st.container(key="psicologo_urg_actions_toolbar"):
            render_table_toolbar(df_cmp_urg_export, "performance_urg_psicologo.csv", "urg_table_psicologo")

        _urg_key_sel = "_".join(sorted(map(str, current_selected_urgs))) if current_selected_urgs else "none"
        urg_table_key = f"urg_table_psicologo_{_urg_key_sel}"
        _urg_key_changed = st.session_state.get("_prev_urg_table_key_psicologo") != urg_table_key
        st.session_state["_prev_urg_table_key_psicologo"] = urg_table_key
    ...
    escolas_tabela_atual = st.session_state.get("escola_table_selection_psicologo__selected_values", [])
    ...
    if df_aluno.empty:
        ...
        if aluno_col in df_aluno_filtrado.columns:
            ...
            alunos_selecionados = st.multiselect(
                ...
                placeholder="Todos",
                key="psicologo_aluno_multiselect",
            )
        ...
        if serie_col in df_aluno_filtrado.columns:
            ...
            series_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="psicologo_serie_multiselect",
            )
        ...
        if turma_col in df_aluno_filtrado.columns:
            ...
            turmas_selecionadas = st.multiselect(
                ...
                placeholder="Todas",
                key="psicologo_turma_multiselect",
            )
        ...
        if df_aluno_filtrado.empty:
            ...
            if not df_aluno_para_exibir.empty:
                ...
                with st.container(key="psicologo_aluno_actions_toolbar"):
                    render_table_toolbar(df_aluno_head, "detalhes_alunos_psicologo.csv", "aluno_table_psicologo")
