...
## Objetivo
Documento operacional para restaurar e alinhar a tela `Vacinação` (`app/app_pages/vacinacao.py`) ao blueprint oficial em caso de regressão visual/funcional.

> Referência-base obrigatória: `docs/similar_screens_blueprint.md`.

## Escopo Funcional
- Seletor temporal mestre (mesmo padrão da Home).
- Filtros de sidebar: Ano, URG, Escola, Tipo, Vacina.
- Indicador total + cards de Vacina clicáveis.
- Tabela de performance por URG (seleção mestre).
- Tabela comparativa por ANO (Vacina).
- Gráficos de distribuição por URG e por Vacina.
- Detalhamento por aluno.

## Fontes de Dados
- `data/DashboardVacinacao.csv`
- `data/DashboardVacinacaoAluno.csv`
- `data/DashboardVacinacaoAno.csv`
- Schemas: `SCHEMA_VACINACAO`, `SCHEMA_VACINACAO_ALUNO`, `SCHEMA_VACINACAO_ANO`.
...
- KPI Vacina: `vacinacao_vacina_multiselect`
...
- Vacina vem de `vacinacao_vacina_multiselect`.
...
- Cards de Vacina: base imune ao próprio filtro (`df_filt_no_vac`) para manter opções visíveis.
- Regra de KPI:
...
6. Validar cards de Vacina (toggle + estilo).
...
- `python -m py_compile app/app_pages/vacinacao.py app/utils/page_helpers.py app/components/sidebar_filters.py`
- `rg -n "render_saedas_aggrid\\(|render_table_toolbar\\(|split_aggrid_footer\\(|vacinacao_vacina_multiselect" app/app_pages/vacinacao.py app/utils/page_helpers.py`
- `rg -n "st\\.dataframe\\(|last_interaction_source|pending_sidebar_|selection-master-table|st-table-with-total" app/app_pages/vacinacao.py app/utils/page_helpers.py`
